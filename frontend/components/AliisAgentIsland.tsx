...
function AliisAgentFAB() {
  const { setOpen } = useAliisAgentContext()
  return (
    <button
      onClick={() => setOpen(true)}
      aria-label="Pregúntale a Aliis"
      className="fixed bottom-[calc(64px+12px+env(safe-area-inset-bottom))] right-4 z-50 md:hidden w-11 h-11 rounded-full bg-foreground text-background flex items-center justify-center shadow-[var(--c-btn-primary-shadow)] border-none cursor-pointer"
    >
